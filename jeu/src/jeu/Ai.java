    ...
    private static void dfs(int deep,Node root,int alpha,int beta,Point p){
        ...
        while(it.hasNext()){  //ajouter les positions possibles dans l'arbre de recherche
            ...
            for(int i=0;i<8;++i){  //
                ...
                if(1<=now.x+dc[i] && now.x+dc[i]<=size && 1<=now.y+dr[i] && now.y+dr[i]<=size && chessBoard[next.y][next.x]==0){  //si position prochaine vide
                    if(!toJudge.contains(next)){  //si dans toJudge
                        toJudge.add(next);  
                    }  
                    else flags[i]=false;  //sinon cette orientation fausse
                }  
            }  
...
//Methode qui permet de verifier les types des pieces actuels( Nombre de pièces consécutives,nombre de case vide, nombre de bouchon...)
public static boolean checkLine(int x, int y, int dx, int dy) {
    int cnt = 1;
    int col = x, row = y;
    while (col + dx > 0 && col + dx <= size && row + dy > 0 && row + dy <= size && chessBoard[row + dy][col + dx] == chessBoard[y][x]) {
        cnt++;
        col += dx;
        row += dy;
    }
